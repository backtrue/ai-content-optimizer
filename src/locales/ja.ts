...
export const jaStrings: UIStrings = {
  ...
  scoreCard: {
    ...
    contentOrganization: 'コンテンツ編成',
    readability: '可読性',
    evidence: '証拠と引用',
    experience: '実務経験',
    freshness: '新鮮さと更新',
    actionability: '実行可能性',
    semanticQuality: 'セマンティック品質',
    credibility: '信頼性',
    trustSignals: '信頼シグナル',
    why: 'Why（問題）',
    how: 'How（方法）',
    what: 'What（解決策）',
    overallScore: '全体戦略スコア',
    structure: '構造',
    strategy: '戦略',
    structureExplanationIntro: '段落レイアウト、可読性、証拠、経験などのコンテンツ構造シグナルは v5 スコアの 40% を占め、記事の骨組みを確保します。',
    strategyExplanationIntro: 'Why/How/What 戦略フレームワークの深さは 60% を占め、コンテンツが読者と検索ニーズに真に対応しているかを反映します。',
    headingStructureExpl: 'H1/H2 階層が明確で、タイトルがトピックに焦点を当てているかを確認し、読者と検索エンジンが素早く概要を把握できるようにします。',
    contentOrganizationExpl: '段落、リスト、表などの構造化要素の使用を評価して、情報が整理され、散らばっていないことを確認します。',
    readabilityExpl: '文の長さと長い段落の比率を測定して、言語の流れが滑らかで段落の長さが適切であることを確認し、読む負担を軽減します。',
    evidenceExpl: 'データ、年号、信頼できる情報源が引用されて主張をサポートし、信頼性を高めているかを確認します。',
    experienceExpl: '一人称の経験、ケーススタディ、実践的な洞察を検出して、EEAT の「経験」シグナルを強化します。',
    freshnessExpl: '更新日が開示され、最新の情報が引用されているかを確認し、古いコンテンツが検索信頼性に影響しないようにします。',
    actionabilityExpl: 'コンテンツがステップ、チェックリスト、または具体的な提案を提供して、読者がすぐに行動できるようにしているかを確認します。',
    semanticQualityExpl: 'HTML モードでは、セマンティックな自然さと段落フォーカスを評価して、キーワード詰め込みを避けます。',
    contentLengthExpl: 'プレーンテキスト モードでは、単語数と段落密度を評価して、包括的なトピック カバレッジを確保し、ペーシングを維持します。',
    whyExpl: 'このトピックが重要な理由を説明し、読者の痛点と文脈を具体的に描写して、信頼性を確立しているかを評価します。',
    howExpl: 'コンテンツが明確なステップ、方法、実行ガイドラインを提供して、読者が何をすべきかを知っているかを評価します。',
    whatExpl: '最終的な主張と行動喚起が明確で、読者が次のステップまたは得られる価値を理解するのに役立つかを確認します。',
    overallScoreExpl: 'Why/How/What の 3 つの側面全体で平均化された戦略パフォーマンス。全体的な説得力と物語の完全性を測定します。'
  },
